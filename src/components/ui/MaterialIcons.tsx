type Props = {
    className?: string
    icon: 'CloseIcon' | 'EmailIcon' | 'FaxIcon' | 'KeyboardArrowDownIcon' | 'KeyboardArrowRightIcon' | 'LaunchIcon' | 'MenuIcon' | 'PhoneIcon' | 'FolderIcon'
}
...
const MaterialIcons = ({ className, icon }: Props) => {
    return (
        <svg className={`mui-icon ${className ? className : ''}`} focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid={icon}>
            ...
            {icon == 'EmailIcon' && <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2m0 14H4V8l8 5 8-5zm-8-7L4 6h16z"></path>}
            ...
            {icon == 'FolderIcon' && <path d="M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2m0 12H4V8h16z"></path>}
        </svg>
    )
}
